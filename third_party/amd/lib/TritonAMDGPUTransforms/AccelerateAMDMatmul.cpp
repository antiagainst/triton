...
public:
  ...
  LogicalResult matchAndRewrite(triton::DotScaledOp dotOp,
    ...
    // For A matrix, 32 K-consecutive elements share the same scale. We'd like
    // to keep the scale values together with the base values in the same warp
    // to avoid cross-warp data exchange. It means we want warpsPerCTA = 1 along
    // the N dimension.
    ...
    // We need to have "matching" encoding between the A matrix and A scale
    // matrix to make sure the scale values needed is in the same warp. So we
